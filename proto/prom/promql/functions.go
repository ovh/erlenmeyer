...
func funcDropCommonLabels(vals []Value, args Expressions, enh *EvalNodeHelper) Vector {
	vec := vals[0].(Vector)
	if len(vec) < 1 {
		return Vector{}
	}
...
func funcCountScalar(vals []Value, args Expressions, enh *EvalNodeHelper) Vector {
	v := vals[0].(Vector)
	if len(v) != 1 {
		return append(enh.out, Sample{
			Point: Point{V: float64(0)},
		})
	}
	return append(enh.out, Sample{
		Point: Point{V: float64(len(v))},
	})
}
